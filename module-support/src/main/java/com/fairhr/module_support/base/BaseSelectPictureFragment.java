...
public abstract class BaseSelectPictureFragment extends FrameworkFragment {
                ...
                .subscribe(new Consumer<Boolean>() {
                    ...
                    public void accept(Boolean granted) throws Exception {
                        if (granted) {
                            ...
                            mCameraUri = SystemAppUtil.openCamera(BaseSelectPictureFragment.this,
                                    sdCardPublicDir,
    ...
    public void cropPicture(final String filePath) {
        mRxPermissions.request(Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE)
                .subscribe(new Consumer<Boolean>() {
                    ...
                    public void accept(Boolean granted) throws Exception {
                        if (granted) {
                            int canCropSize = BitmapUtils.pictureCanCropSize(mAttachActivity.getApplicationContext(), filePath);
                            File tarFile = new File(AndroidFileUtils.getSDAppFileDir(mAttachActivity.getApplicationContext(), Environment.DIRECTORY_DCIM), FileUtils.creatImageFileName(FileUtils.getFileFormat(filePath)));
                            mCropUri = SystemAppUtil.cropPhoto(BaseSelectPictureFragment.this, canCropSize, new File(filePath), tarFile, MY_CROP_PICTURE_REQUEST);
                        }
                    }
                });
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == MY_PHOTO_ALUM_REQUEST && resultCode == Activity.RESULT_OK && data != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                FileUtils.sdkQUriToFilePath(mAttachActivity.getApplicationContext(), fileUri, Environment.DIRECTORY_PICTURES, MAX_SIZE).subscribeOn(Schedulers.io())
                ...
                File file = FileUtils.getFileByUri(mAttachActivity.getApplicationContext(), fileUri);
                if (file != null && file.exists() && file.length() > 0) {
                    if (file.length() > MAX_SIZE) {
                        File tarFile = new File(AndroidFileUtils.getSDAppFileDir(mAttachActivity.getApplicationContext(), Environment.DIRECTORY_DCIM), FileUtils.getFileName(file.getAbsolutePath()));
                        BitmapUtils.compressToAssignSize(mAttachActivity.getApplicationContext(), file.getAbsolutePath(), tarFile.getAbsolutePath(),
        ...
        if (requestCode == MY_CAMERA_REQUEST && resultCode == Activity.RESULT_OK) {
            if (mCameraUri == null)
                return;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                FileUtils.sdkQUriToFilePath(mAttachActivity.getApplicationContext(), mCameraUri, Environment.DIRECTORY_PICTURES, MAX_SIZE).subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(new Observer<String>() {
                            @Override
                            public void onSubscribe(Disposable d) {
                ...
                File file = FileUtils.getFileByUri(mAttachActivity.getApplicationContext(), mCameraUri);
                if (file.exists() && file.length() > 0) {
                    if (file.length() > MAX_SIZE) {
                        ...
                        SystemUtil.scanFile(mAttachActivity.getApplicationContext(), file.getAbsolutePath());
                    }
                }
            }
            return;
        ...
        if (requestCode == MY_CROP_PICTURE_REQUEST && resultCode == Activity.RESULT_OK) {
            ...
            if (file != null && file.exists() && file.length() > 0) {
                if (file.length() > MAX_SIZE) {
                    BitmapUtils.compressToAssignSize(mAttachActivity.getApplicationContext(), file.getAbsolutePath(), MAX_SIZE, new Observer<String>() {
                        @Override
                        public void onSubscribe(Disposable d) {
